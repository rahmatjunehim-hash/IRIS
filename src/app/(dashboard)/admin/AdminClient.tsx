...
export default function AdminClient() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      {/* Top Banner (Apple Dark Glass Style) */}
      <div className="bg-slate-900 text-white rounded-3xl p-6 sm:p-8 shadow-apple-float border border-slate-800 relative overflow-hidden">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-5">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 border border-white/15 text-emerald-300 text-[11px] font-semibold mb-3 backdrop-blur-md">
            ...
            <p className="text-slate-400 text-xs sm:text-sm mt-1 max-w-2xl font-normal leading-relaxed">
          ...
          <div className="flex gap-2 shrink-0">
            <button
              ...
              className="apple-press px-4 py-2.5 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white font-medium text-xs flex items-center gap-2 shadow-apple-subtle transition cursor-pointer"
            >
      ...
      <div className="apple-segmented-container p-1 rounded-2xl flex items-center gap-1 w-fit">
        <button
      ...
      {activeTab === "OVERVIEW" && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-white/85 backdrop-blur-xl p-5 sm:p-6 rounded-3xl border border-white/60 shadow-apple-card">
              <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Cabang Resmi</span>
              <div className="mt-3 flex items-baseline gap-2">
                <span className="text-3xl font-bold tracking-tight text-slate-900 font-mono tabular-nums">4</span>
                <span className="text-xs text-slate-500 font-medium">Purwokerto, Cilacap, Wonosobo, Purbalingga</span>
              </div>
            </div>
            <div className="bg-white/85 backdrop-blur-xl p-5 sm:p-6 rounded-3xl border border-white/60 shadow-apple-card">
              <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Akun Pengguna</span>
              <div className="mt-3 flex items-baseline gap-2">
                <span className="text-3xl font-bold tracking-tight text-slate-900 font-mono tabular-nums">{users.length}</span>
                <span className="text-xs text-slate-500 font-medium">7 Role Operasional</span>
              </div>
            </div>
            <div className="bg-white/85 backdrop-blur-xl p-5 sm:p-6 rounded-3xl border border-white/60 shadow-apple-card">
              <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Status Modul</span>
            ...
            <div className="bg-white/85 backdrop-blur-xl p-5 sm:p-6 rounded-3xl border border-white/60 shadow-apple-card">
              <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">WhatsApp Gateway</span>
              <div className="mt-3 flex items-baseline gap-2">
                <span className="text-lg font-bold text-slate-900">Fonnte</span>
                <span className="text-xs font-semibold text-emerald-700 bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20">Aktif</span>
              </div>
            </div>
          </div>

          {/* Quick Access to Public Pages */}
          <div className="bg-white/85 backdrop-blur-xl p-6 sm:p-7 rounded-3xl border border-white/60 shadow-apple-card">
            <h3 className="font-bold tracking-tight text-slate-900 text-base mb-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
              {[
                ...
                <div key={c.slug} className="p-4 rounded-2xl bg-black/[0.02] border border-slate-200/60 space-y-2.5">
              ...
              <tbody className="divide-y divide-slate-100">
                {users.map((u) => (
                  <tr key={u.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="py-3.5 px-3 font-mono tabular-nums text-slate-600">@{u.username}</td>
                    ...
                    <td className="py-3.5 px-3 text-slate-700">{u.cabangName}</td>
                    <td className="py-3.5 px-3">
                      <span
                      ...
                      {u.role !== "SUPER_ADMIN" && (
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button
                            ...
                            className="apple-press text-[11px] font-medium px-2 py-1 rounded-xl text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition cursor-pointer"
                            title="Hapus akun"
                          >
                            Hapus
                          </button>
      ...
      {activeTab === "CABANG" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {[
            ...
            <div key={c.kode} className="bg-white/85 backdrop-blur-xl rounded-3xl p-6 sm:p-7 border border-white/60 shadow-apple-card space-y-4">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3.5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400 font-mono">Kode Cabang: {c.kode}</p>
                  </div>
                </div>
              ...
              <div className="text-xs text-slate-600 space-y-1.5 font-normal">
                <p><strong className="text-slate-900 font-semibold">Alamat:</strong> {c.alamat}</p>
                <p><strong className="text-slate-900 font-semibold">Telepon / WA:</strong> <span className="font-mono tabular-nums">{c.telepon}</span></p>
              </div>

              <div className="pt-2.5 border-t border-slate-100 flex items-center gap-3 text-xs">
                <a
                  href={`/display/${c.slug}`}
                  target="_blank"
                  className="apple-press py-1.5 px-3.5 rounded-xl bg-slate-100 text-slate-700 hover:bg-slate-200/80 transition font-medium flex items-center gap-1.5"
                >
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-md flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white/95 backdrop-blur-2xl rounded-3xl p-6 sm:p-7 max-w-md w-full border border-white/60 shadow-apple-float space-y-5">
            <h3 className="font-bold tracking-tight text-slate-900 text-base">
            ...
            <form onSubmit={handleAddUser} className="space-y-3.5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-2.5 text-xs bg-black/[0.02] border border-slate-200 rounded-2xl font-mono focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 focus:outline-none transition"
                />
                <span className="text-[10px] text-slate-400 mt-1 block">Default jika dikosongkan: password123</span>
              </div>
              ...
              <div className="pt-3.5 border-t border-slate-100 flex items-center justify-end gap-2.5">
                <button
                  ...
                  className="apple-press py-2 px-4 rounded-2xl border border-slate-200 text-slate-700 text-xs font-medium hover:bg-slate-50 transition cursor-pointer"
                >
                  Batal
                </button>
                <button
                  type="submit"
                  className="apple-press py-2 px-4 rounded-2xl bg-emerald-800 hover:bg-emerald-900 text-white text-xs font-medium shadow-apple-subtle transition cursor-pointer"
                >
                  Simpan Akun
                </button>
              </div>
